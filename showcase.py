import pandas as pd
import streamlit as st
import plotly.express as px

st.set_page_config(layout="wide")

st.title("Proyek Kelompok Praktikum Analisa Big Data Ekonomika (ELA6105)")

st.markdown('''
            Berikut ini adalah arsip Proyek Kelompok mata kuliah
            Praktikum Analisa Big Data Ekonomika (ELA6105) yang
            diselenggarakan oleh Program Studi Ekonomi Pembangunan,
            Fakultas Ekonomi dan Bisnis, Universitas Trisakti.

            Kuliah ini mulai dilaksanakan melalui kegiatan
            [_Training of Trainer_](https://kuikk.trisakti.ac.id/news/training-of-trainers-tot-praktikum-analisis-big-data-ekonomi-program-studi-s1-ekonomi-pembangunan-feb-usakti/)
            sebagai bagian dari pelaksanaan mata kuliah Praktikum
            dalam Kurikulum Program Studi Ekonomi Pembangunan. Sebagai
            dosen koordinator ditunjuk ibu
            [Sri Yani Kusumastuti, SE, MSI](https://ep.feb.trisakti.ac.id/tentang/dosen-prodi-ep/).
            Kuliah ini dilaksanakan secara _team teaching_ dengan
            dosen pengajar dari Program Studi Sarjana Sistem
            Informasi, Fakultas Teknologi Industri, bapak [Anung B
            Ariwibowo, SKom, MKom](https://sistem-informasi.fti.trisakti.ac.id/profil/dosen/)
            ''')

st.write('# Semester Gasal 2025/2026')
st.write('Group names are sorted by alphabetical order.')
st.write('## Kelas Beasiswa-4.3')
st.markdown('''
- [Man Jadda Wajada](https://man-jadda-wajada-real.streamlit.app/)
- [Obidebug](https://yupihengker.streamlit.app/). [initial version](https://obidebug-ekxzrnzmzswux2sosgfzc7.streamlit.app/)
- [Remaja Ashabul Kahfi](https://remaja-ashabul-kahfi-myhesrogsc9gvvrpw7hcrk.streamlit.app/)
- [Wallahi We Are Finished](https://wallahi-we-re-finished.streamlit.app/)
''')

st.write('## Kelas KKA-5.5')
st.write('After presentation')
st.markdown('''
1. [AADC - Apa Aja Dicodingin](https://aadc-apa-aja-dicodingin-kwgsjbnfpfzh8wxslnjb6x.streamlit.app/). Previous [version](https://aadc-apa-aja-dicodingin-2g7ucj3hhp59nvscmr4pu2.streamlit.app/)
2. [Koding Kiding](https://koding-kiding.streamlit.app/)
3. [Mafia Ngoding](https://mafia-ngoding.streamlit.app/)
4. [Mager Code](https://magercoder.streamlit.app/)
5. [Lebah Ganteng](https://lebah-ganteng.streamlit.app/)
6. [Rajin Ibadah](https://rajinibadah-mhomyv6s2gbhetn4zkyypy.streamlit.app/)
7. [Aku Bukan Anaconda](https://aku-bukan-anaconda-prakbigdata.streamlit.app/)
''')

st.write('## Kelas KKA-6.4')
st.markdown('''
- [Kelompok1](https://kelompok1-prakbigdata.streamlit.app/)
- [Kelompok Dua](https://kelompokdua.streamlit.app/)
''')

st.write("# Semester Gasal 2024/2025")
st.markdown('''
- [Kelompok Bunga Abadi](https://tugaskelompok.streamlit.app/)
- [Kelompok PasmingBased](https://pasmingbased-part1.streamlit.app/)
- [Kelompok Mercon Jontor](https://mercon-jontor.streamlit.app/)
- [Kelompok GoFooodBoleeeh](https://gofooodboleeeh.streamlit.app/)
            (Yes, with triple `o` inf _Foood_ and triple `e` in
            _Boleeeh_)
- [Kelompok Dududada](https://almanacproject.streamlit.app/)
- [Kelompok TimPemburu](https://timpemburu.streamlit.app/)
- [Kelompok MinyakHitam](https://minyakhitam.streamlit.app/)
''')

st.write("# Semester Gasal 2023/2024")
st.markdown('''
- [Proyek Ambyst](https://proyekambyst.streamlit.app/)
- [Fantastic Airs](https://fantasticairs-4nhazektex5gh7zwsipn8x.streamlit.app/)
- [Four Dashboard](https://four-dashboard.streamlit.app/)
- [Watermelon Project](https://watermelonprojecthappinessindex.streamlit.app/)
            ''')

